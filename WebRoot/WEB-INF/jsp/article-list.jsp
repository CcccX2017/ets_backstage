...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/js/Hui-iconfont/1.0.8/iconfont.css" />
			...
			<tbody style="text-align: center;">
				<c:forEach items="${list}" var="val" varStatus="state">
					...
					<td style="text-align: center;" class="text-l">
						...
						class="text-primary"
						onClick="article_edit('公告内容','ariticleDetail.action?id=${val.anId}&author=${val.admin.adUsername}','${val.anId}')"
						title="查看内容">${val.title}</a>
					</td>
					<td>${val.admin.adUsername}</td>
					<td><fmt:formatDate value="${val.createtime}" pattern="yyyy-MM-dd  HH:mm:ss"/></td>
					<td><fmt:formatDate value="${val.updatetime}" pattern="yyyy-MM-dd  HH:mm:ss"/></td>
					<td class="td-status">
						<c:if test="${val.state==1}">
...
<!--_footer 作为公共模版分离出去-->
<script type="text/javascript" src="${pageContext.request.contextPath}/js/jquery/1.9.1/jquery.min.js"></script> 
...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/datatables/1.10.0/jquery.dataTables.min.js"></script> 
<script type="text/javascript" src="${pageContext.request.contextPath}/js/laypage/1.2/laypage.js"></script>
...
function article_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'GET',
			url: 'announcement/delAnnouncement.action?id='+id,
			dataType: 'json',
